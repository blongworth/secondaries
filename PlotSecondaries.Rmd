...
#out.s <- subset(out.s, tp_date_pressed > as.POSIXct("2013-02-01 EDT"))
...
* Mean Sigma for AMS1 is `r mean(out.s$sigma[out.s$system=="AMS1"])`.
* Mean Sigma for CFAMS is `r mean(out.s$sigma[out.s$system=="CFAMS"])`.
* No correlation to size or date run.
...
#sigma boxplots
p <- ggplot(out.s, aes(name, sigma)) + geom_hline(yintercept=0) + geom_boxplot() + geom_jitter()
#p <- qplot(name, sigma, data=out.m, geom=c("boxplot", "jitter"))
p + facet_grid(system ~ .) + ggtitle(paste("Sigma Boxplots, ", system, ", ", from)) +
...
Looking at fractional error within samples to get an idea of system precision. This is error / fm. Internal (counting) error and external error compared. I am not sure that these errors are comparable between systems, so please take with a grain of salt.
