...
monthsum <- ddply(out.s, .(month, system), summarize,
                fm.e = mean(fm_consensus),
                fm = mean(f_modern),
                fm.s = sd(f_modern),
                sig = mean(sigma),
...
ggplot(data=monthsum, aes(x=month, y=ferr, fill=system)) + 
  geom_bar(stat="identity", position=position_dodge(), colour="black") +
  coord_cartesian(ylim = c(0.002, 0.0026)) +
  ggtitle("reported error of OX-I by month")

ggplot(data=out.s, aes(x=month, y=fexterr/finterr, fill=system)) + 
  geom_bar(stat="identity", position=position_dodge(), colour="black") + 
  coord_cartesian(ylim = c(1, 2)) +
  ggtitle("Error ratio by month")

ggplot(data=out.s, aes(x=month, y=fexterr, fill=system)) + 
  geom_bar(stat="identity", position=position_dodge(), colour="black") + 
#  coord_cartesian(ylim = c(1, 2)) +
  ggtitle("Error ratio by month")
