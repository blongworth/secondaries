...
out <- std %>%
  ...
  mutate(year = year(tp_date_pressed),
         month = month(tp_date_pressed),
...
* Data are fraction of all secondaries > 3 sigma

```{r}

out.s <- out %>% group_by(year, month, lab, system) %>% 
  summarise(hisig = sum(abs(sigma) > 3),
            hisigav = sum(abs(sigav) > 3),
            count = n()) %>%
...
"hisig" is number of secondaries > 3 sigma, columns with "av" include
added variance. Columns ending in "f" are fraction greater than 3 sigma.
...
out %>% 
  ...
  mutate(hisigf = hisig/count,
         hisigavf = hisigav/count) %>%
  kable(caption = "Secondaries > 3 sigma, 2016 to present")
```

### Plots

* Bars are fraction of secondaries greater than 3 sigma by month and system
...
ggplot(out.s, aes(date, hisigf)) + 
  ...
  facet_grid(lab ~ .) +
    ggtitle("Fraction of secondaries > 3 sigma")
```
...
ggplot(out.s, aes(date, hisigavf)) + 
  ...
  facet_grid(lab ~ .) +
  theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
    ggtitle("Fraction of secondaries > 3 sigma", subtitle = "With added variance of 0.026 * Fm")
```
