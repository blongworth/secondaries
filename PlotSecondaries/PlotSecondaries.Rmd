...
Analysis of performance of all secondary standards for `r sys` from `r from` to `r to`. Sample size is from `r size[1]` to `r size[2]`umol, outliers have been removed by sigma (less than `r msig`) and normalized Fm (less than `r mnfm`), and only secondaries Fm > .1 used.
