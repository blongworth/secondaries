...
out.s <- filter(out, primary == FALSE, # rec_num != 113385, #don't use normalizing ox-I
                ...
                abs(sigma) < 3, #Select reasonable sigmas
                abs(normFm) < 0.03, #Select reasonable Fm
                frep_err < 0.10
              ) %>%
  mutate(rep_err_cor = calcRE(f_modern, rep_err, 0.0026),
...
#Sigma histogram
ggplot(out.s, aes(sigma_rec)) + geom_histogram() + 
  facet_grid(system ~ .) + 
  ggtitle(paste("Sigma Histogram (w/residual error), ", sys, ", ", from))
```
...
#sigma boxplots
p <- ggplot(out.s, aes(name, sigma_rec, color=process)) +
  geom_hline(yintercept=0) + 
  geom_hline(yintercept=1) + 
  geom_hline(yintercept=-1) + 
  geom_boxplot() + geom_jitter()
p + facet_grid(system ~ .) + 
  ggtitle(paste("Sigma Boxplots (res err), ", sys, ", ", from)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
...
#sigma boxplots
p <- ggplot(out.s, aes(system, sigma_rec)) + geom_hline(yintercept=0) + 
  geom_boxplot() + geom_jitter(aes(color=process))
p + ggtitle(paste("Sigma Boxplots for all secondaries by system (res err)")) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
...
#ratio vs sigma
p <- ggplot(data=out.s, aes(f_modern, sigma_rec, color=system))
p + geom_hline(yintercept=0) + geom_point(aes(shape=process)) + stat_smooth(method="lm", se=FALSE) +
  ggtitle(paste("Sigma vs Fm(res err), ", sys, ", ", from))
  #scale_colour_manual(values=cbPalette)
